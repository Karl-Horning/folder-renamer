...
describe("moveImageCountAndDate", () => {
    ...
    it("moves both an image count and a date to the end, count before date", () => {
        expect(moveImageCountAndDate("(2024-01-05) Holiday (x238) Snaps")).toBe(
            "Holiday  Snaps (x238) (2024-01-05)",
        );
    });
...
describe("cleanupPunctuation", () => {
    ...
    it("removes a trailing comma before metadata in brackets", () => {
        expect(cleanupPunctuation("Holiday Snaps, (x238)")).toBe("Holiday Snaps (x238)");
    });
...
describe("movePrefixesToEnd", () => {
    ...
    it("moves a comma-separated prefix to the end in square brackets", () => {
        expect(movePrefixesToEnd("MyPhotos, Holidays, Holiday snaps", prefixes)).toBe(
            "Holidays, Holiday snaps [MyPhotos]",
        );
    });

    it("moves a space-separated prefix to the end in square brackets", () => {
        expect(movePrefixesToEnd("FamilyPhotos Holidays, Holiday snaps", prefixes)).toBe(
            "Holidays, Holiday snaps [FamilyPhotos]",
        );
    });

    it("matches a prefix case-insensitively, using the prefix list's casing in the output", () => {
        expect(movePrefixesToEnd("myphotos Holidays", prefixes)).toBe("Holidays [MyPhotos]");
    });
    ...
    it("only matches a prefix at the start of the name", () => {
        expect(movePrefixesToEnd("Holidays, MyPhotos snaps", prefixes)).toBe(
            "Holidays, MyPhotos snaps",
        );
    });
});
