...
import { applyReplacePatterns, hasAnyPatterns, initReplacePatterns } from "./replacePatterns.js";
...
describe("applyReplacePatterns", () => {
    ...
    it("strips bracketed tags regardless of case", () => {
        expect(applyReplacePatterns("My Comic (Digital-HD)")).toBe("My Comic");
        expect(applyReplacePatterns("My Comic (digital-hd)")).toBe("My Comic");
    });
    ...
    it("strips known download phrases regardless of case or punctuation", () => {
        expect(applyReplacePatterns("Download Link; Something")).toBe("Something");
        expect(applyReplacePatterns("Download link Something")).toBe("Something");
        expect(applyReplacePatterns("Download zip Something")).toBe("Something");
        expect(applyReplacePatterns("Download Mirror; Something")).toBe("Something");
        expect(applyReplacePatterns("Download ZIP file; Something")).toBe("Something");
    });

    it("strips future/pre-release phrases regardless of case or brackets", () => {
        expect(applyReplacePatterns("Something (future release)")).toBe("Something");
        expect(applyReplacePatterns("Something Future Release")).toBe("Something");
        expect(applyReplacePatterns("Something (pre-release)")).toBe("Something");
        expect(applyReplacePatterns("Something Pre-release")).toBe("Something");
    });

    it("strips 'Upcoming Release' and 'Unreleased' suffixes with or without a dash", () => {
        expect(applyReplacePatterns("Something, Upcoming Release")).toBe("Something");
        expect(applyReplacePatterns("Something, - Upcoming Release")).toBe("Something,");
        expect(applyReplacePatterns("Something, Unreleased")).toBe("Something");
    });

    it("only capitalises 'set' as a whole word", () => {
        expect(applyReplacePatterns("This is a set of things")).toBe("This is a Set of things");
        expect(applyReplacePatterns("Reset Assets Preset")).toBe("Reset Assets Preset");
    });
    ...
    it("normalises ' - ', ' • ', and ' ¦ ' separators to ', '", () => {
        expect(applyReplacePatterns("Something - Else")).toBe("Something, Else");
        expect(applyReplacePatterns("Something • Else")).toBe("Something, Else");
        expect(applyReplacePatterns("Something ¦ Else")).toBe("Something, Else");
    });
...
describe("hasAnyPatterns", () => {
    it("reports no patterns loaded when both pattern files are empty", async () => {
        const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), "replacepatterns-empty-test-"));
