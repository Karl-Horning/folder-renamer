...
describe("transformName", () => {
    ...
    beforeAll(async () => {
        ...
        const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), "transformname-test-"));
        await fs.writeFile(
    ...
    it("normalises an image count and moves it to the end", () => {
        expect(transformName("Holiday Snaps 238x", prefixes)).toBe("Holiday Snaps (x238)");
    });
    ...
    it("moves a known prefix to the end in square brackets", () => {
        expect(transformName("MyPhotos Holiday Snaps", prefixes)).toBe("Holiday Snaps [MyPhotos]");
    });

    it("combines count, date, and prefix handling in one pass", () => {
        expect(transformName("MyPhotos Holiday Snaps 238x 2024-01-05", prefixes)).toBe(
            "Holiday Snaps (x238) (2024-01-05) [MyPhotos]",
        );
    });

    it("strips a known bracketed tag and cleans up the resulting punctuation", () => {
        expect(transformName("Holiday Snaps (Scanner) (2 covers)", prefixes)).toBe("Holiday Snaps");
    });

    it("removes stray empty brackets left behind by other transforms", () => {
        expect(transformName("Holiday Snaps ()", prefixes)).toBe("Holiday Snaps");
    });
